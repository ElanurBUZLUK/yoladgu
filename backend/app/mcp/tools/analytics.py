...
class AnalyticsTool(BaseMCPTool):
    ...
    async def execute(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Temel istatistikler
        basic_stats = self._calculate_basic_stats(recent_attempts)
        
        # Analiz tipine göre detaylı analiz
        detailed_analysis = {}
        
        if analysis_type == "level_assessment":
            detailed_analysis = await self._assess_level(recent_attempts, subject)
        elif analysis_type == "error_pattern":
            detailed_analysis = await self._analyze_error_patterns(recent_attempts, subject)
        elif analysis_type == "progress_tracking":
            detailed_analysis = await self._track_progress(recent_attempts, subject)
        elif analysis_type == "similarity_analysis":
            detailed_analysis = await self._find_similar_students(user_id, recent_attempts, subject)
        
        # Öneriler üret
        recommendations = self._generate_recommendations(basic_stats, detailed_analysis, subject)
        ...
        return {
            ...
            "analysis": {
                ...
                "next_actions": self._suggest_next_actions(basic_stats, detailed_analysis)
            },
            "metadata": {
