...
class CollaborativeFilterEngine:
    ...
    def add_interaction(
        self, user_id: int, item_id: int, rating: float, implicit: bool = True
    ):
        ...
        try:
            ...
            logger.info(
                "interaction_added", user_id=user_id, item_id=item_id, rating=rating
            )
    ...
    def get_recommendations(
        self, user_id: int, n_recommendations: int = 10, exclude_seen: bool = True
    ) -> List[Dict]:
        ...
        try:
            ...
            logger.info(
                "recommendations_generated", user_id=user_id, count=len(recommendations)
            )
    ...
    def get_similar_items(self, item_id: int, n_similar: int = 10) -> List[Dict]:
        ...
        try:
            ...
            similarities = cosine_similarity(
                item_profile, self.nmf_model.components_.T
            )[0]
    ...
    def _build_interaction_matrix(self, interactions: List[Dict]):
        ...
        self.interaction_matrix = csr_matrix(
            (data, (rows, cols)), shape=(len(unique_users), len(unique_items))
        )
    ...
    def _fallback_recommendations(
        ...
        try:
            ...
            for item_id in selected_items:
                recommendations.append(
                    {"item_id": item_id, "score": 0.5, "method": "random_fallback"}
                )
    ...
    def _save_interactions(self, interactions: List[Dict]):
        ...
        try:
            self.redis.setex(
                ...
                86400 * 7,
                json.dumps(interactions),  # 1 week
            )
        except Exception as e:
    ...
    def _save_model(self):
        ...
        try:
            model_data = {
                "nmf_model": pickle.dumps(self.nmf_model).hex()
                if self.nmf_model
                else None,
                "svd_model": pickle.dumps(self.svd_model).hex()
                if self.svd_model
                else None,
                ...
                "interaction_matrix": pickle.dumps(self.interaction_matrix).hex()
    ...
    def _load_model(self):
        ...
        try:
            ...
            if model_data.get("interaction_matrix"):
                self.interaction_matrix = pickle.loads(
                    bytes.fromhex(model_data["interaction_matrix"])
                )
    ...
    def reset(self):
        ...
        try:
            ...
            # Clear Redis
            self.redis.delete(
                self.model_key, self.interaction_matrix_key, self.stats_key
            )
