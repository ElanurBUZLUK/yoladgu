...
class TransactionManager:
    ...
    def execute(self) -> bool:
        ...
        try:
            for i, operation in enumerate(self.operations):
                logger.debug("executing_operation", 
                           transaction_id=self.transaction_id,
    ...
    def _compensate(self, executed_operations: List[TransactionOperation]):
        ...
        logger.info("starting_compensation", 
                   transaction_id=self.transaction_id,
        ...
        for operation in reversed(executed_operations):
            if operation.compensate:
                try:
                    ...
                    logger.debug("compensation_completed", 
                               transaction_id=self.transaction_id,
                               operation_name=operation.name)
                except Exception as e:
...
def distributed_transaction(db: Session):
    ...
    try:
        ...
        if tx.status == TransactionStatus.COMMITTED:
            db.commit()
            logger.info("postgres_transaction_committed", 
                       transaction_id=tx.transaction_id)
        else:
            db.rollback()
            logger.info("postgres_transaction_rolled_back", 
                       transaction_id=tx.transaction_id)
            
    except SQLAlchemyError as e:
        db.rollback()
        logger.error("postgres_transaction_error", 
                    transaction_id=tx.transaction_id,
                    error=str(e))
        raise
    except Exception as e:
        db.rollback()
        logger.error("distributed_transaction_error", 
                    transaction_id=tx.transaction_id,
                    error=str(e))
        raise
...
class StudentResponseTransaction:
    ...
    def execute_student_response(
        ...
        try:
            ...
            logger.error("student_response_transaction_failed", 
                        student_id=student_id,
                        question_id=question_id,
                        error=str(e))
            return False
