    ...
    async def start_background_task(
        ...
        async def managed_task():
            ...
            while not self.shutdown_event.is_set() and retries <= max_retries:
                try:
                    logger.info(
                        "starting_background_task", name=name, attempt=retries + 1
                    )
                    ...
                    if task in done:
                        try:
                            ...
                            if retries <= max_retries:
                                ...
                                logger.error(
                                    "background_task_max_retries_exceeded", name=name
                                )
                                break

                except Exception as e:
                    logger.error(
                        "background_task_manager_error", name=name, error=str(e)
                    )
                    break
                ...
                for name, process_info in self.processes.items():
                    ...
                    if task:
                        if task.done():
                            if task.exception():
                                logger.error(
                                    "background_task_health_check_failed",
                                    name=name,
...
def health_check_endpoint():
    ...
    try:
        ...
        healthy_processes = sum(
            1 for p in status.values() if p.get("status") == "running"
        )
