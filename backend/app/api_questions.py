...
async def questions_next(
    ...
    await db.execute(
        insert(Event).values(
            user_id=user.id,
            event_type="question_exposure",
            payload=json.dumps({"question_id": body.question_id, "score": final}),
        )
    )
    await db.commit()
...
async def questions_submit(
    ...
    await db.execute(
        insert(Event).values(
            user_id=user.id,
            event_type="question_outcome",
            payload=json.dumps({"question_id": question_id, "correct": correct}),
        )
    )
    await db.commit()
    return {"ok": True}
