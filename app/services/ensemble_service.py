...
class EnhancedEnsembleScoringService:
    def __init__(self, redis_client=None):
        ...
        self.weights = (
            ...
            if hasattr(settings, "ENSEMBLE_WEIGHTS")
            else {
        ...
        try:
            ...
            for question in candidate_questions:
                ...
                try:
                    scores[
                        "river_score"
                    ] = self.river_learner.predict_success_probability(
                ...
                try:
                    if len(candidate_questions) > 1:
                        ...
                        if selected_id == question_id:
                            scores["bandit_score"] = selection_info.get(
                                "ucb_value", 0.5
                            )
                        else:
                            scores["bandit_score"] = selection_info.get(
                ...
                try:
                    ...
                    if user_id:
                        cf_recommendations = (
                            self.collaborative_filter.get_recommendations(
                                user_id, n_recommendations=20, exclude_seen=True
                            )
                        )
                        cf_scores = {
                            rec["item_id"]: rec["score"] for rec in cf_recommendations
                        }
                ...
                try:
                    ...
                    recent_questions = (
                        context.get("recent_questions", []) if context else []
                    )
                    scores[
                ...
                try:
                    scores["neo4j_similarity"] = self.calculate_neo4j_similarity_score(
                        question_id,
                        context.get("recent_question_ids", []) if context else [],
                    )
                except Exception as e:
            ...
            logger.info(
                "ensemble_scoring_completed",
                n_questions=len(scored_questions),
                top_score=scored_questions[0]["ensemble_score"]
                if scored_questions
                else 0,
            )
    ...
    def update_models_with_feedback(
        self,
        user_features: Dict,
        question_features: Dict,
        is_correct: bool,
    ...
    def _calculate_diversity_bonus(
        self, question: Dict, recent_questions: List[str]
    ) -> float:
        ...
        try:
            ...
            for recent_q in recent_questions[-3:]:  # Son 3 soru
                common_words = set(question_content.lower().split()) & set(
                    recent_q.lower().split()
                )
                overlap_ratio = len(common_words) / max(
                    len(question_content.split()), 1
                )
            ...
            for recent_emb in recent_embeddings:
                if recent_emb and question_embedding:
                    ...
                    similarity = np.dot(recent_emb, question_embedding) / (
                        np.linalg.norm(recent_emb) * np.linalg.norm(question_embedding)
                        + 1e-8
                    )
    ...
    async def calculate_enhanced_embedding_similarity_score(
        self,
        question_id: int,
        ...
        try:
            ...
            intersection_ratio = (
                len(intersection) / len(student_recent_set) if student_recent_set else 0
            )
            ...
            top_similarities = [
                result["similarity_score"] for result in similar_results[:5]
            ]
            ...
            logger.debug(
                "enhanced_embedding_similarity_calculated",
                question_id=question_id,
            ...
            logger.error(
                "enhanced_embedding_similarity_error",
                question_id=question_id,
                error=str(e),
            )
            return 0.5
    ...
    def calculate_neo4j_similarity_score(
        self, question_id: int, student_recent_question_ids: List[int]
    ) -> float:
    ...
    def calculate_ensemble_score(
        ...
        try:
            ...
            skill_mastery_score = self.calculate_skill_mastery_score(
    ...
    def filter_questions_by_thresholds(
        self,
        questions: List[Dict],
        student_level: float,
        student_recent_performance: float,
    ...
    def adjust_weights_dynamically(
        self, student_performance: float, question_count: int
    ) -> None:
...
def filter_questions_by_thresholds(
    questions: List[Dict], student_level: float, student_recent_performance: float
) -> List[Dict]:
    """Threshold'lara göre soruları filtrele"""
...
async def calculate_diversity_score(
    question_id: int, recent_question_ids: List[int]
) -> float:
...
async def get_adaptive_weights(
    student_performance: float, student_level: int
) -> Dict[str, float]:
