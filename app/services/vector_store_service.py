...
class VectorStoreService:
    ...
    async def store_embedding(
        ...
        try:
            if len(embedding) != self.embedding_dim:
                raise ValueError(f"Embedding boyutu {len(embedding)}, beklenen {self.embedding_dim}")
            ...
            async with self.pool.acquire() as conn:
                await conn.execute("""
                    ...
                    DO UPDATE SET 
                        ...
                        updated_at = NOW()
                """, question_id, embedding, content, json.dumps(metadata), 
    ...
    async def similarity_search(
        ...
        try:
            if len(query_embedding) != self.embedding_dim:
                raise ValueError(f"Query embedding boyutu {len(query_embedding)}, beklenen {self.embedding_dim}")
            ...
            if filters:
                ...
                if 'exclude_question_ids' in filters:
                    ...
                    if excluded_ids:
                        placeholders = ','.join([f'${param_counter + i}' for i in range(len(excluded_ids))])
            ...
            index_hint = "/*+ IndexScan(question_embeddings question_embeddings_embedding_idx) */" if use_index_scan else ""
            ...
            logger.debug("similarity_search_completed", 
    ...
    async def get_embedding_by_question_id(self, question_id: int) -> Optional[List[float]]:
    ...
    async def delete_embedding(self, question_id: int) -> bool:
        ...
        try:
            ...
            logger.error("delete_embedding_error", question_id=question_id, error=str(e))
            return False
