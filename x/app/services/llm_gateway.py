...
class LLMGatewayService:
    ...
    async def initialize(self):
        ...
        if self.use_mcp:
            try:
                self.mcp_connected = await mcp_client.connect()
                if self.mcp_connected:
                    logger.info("✅ MCP LLM Gateway connected")
                else:
                    logger.warning("⚠️ MCP connection failed, using direct LLM")
                    self.use_mcp = False
...
llm_gateway = LLMGatewayService(use_mcp=True)
