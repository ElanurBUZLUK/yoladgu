...
async def get_comprehensive_health(
    include_detailed_metrics: bool = Query(default=False),
    ...
    try:
        ...
        # Generate alerts
        alerts = _generate_system_alerts(services, system_metrics, database_health, ml_models)
...
async def run_system_diagnostics(
    test_suite: str = Query(default="basic", pattern="^(basic|full|ml_models|database|network)$"),
...
async def get_ml_models_status():
    ...
    try:
        ...
        return {
            ...
            "overall_ml_health": "healthy" if all(m.status == "healthy" for m in models_status) else "degraded",
...
def _generate_system_alerts(services, system_metrics, database_health, ml_models) -> List[Dict[str, Any]]:
...
async def _get_performance_metrics(minutes: int, endpoint_filter: Optional[str]) -> List[PerformanceMetrics]:
    return []

def _calculate_performance_summary(performance_data: List[PerformanceMetrics]) -> Dict[str, Any]:
...
def _get_resource_thresholds() -> Dict[str, Dict[str, float]]:
    return {"cpu": {"warning": 70.0, "critical": 90.0}, "memory": {"warning": 80.0, "critical": 95.0}}
...
def _get_recent_logs(level: str, count: int, service: Optional[str]) -> List[Dict[str, Any]]:
    return []
...
async def _generate_diagnostic_report(test_suite: str, tests: List[DiagnosticTest]):
    ...
    logger.info("diagnostic_report_generated", test_suite=test_suite, test_count=len(tests))