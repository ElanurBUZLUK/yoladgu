...
class OfflineBatchScheduler:
    ...
    async def daily_embedding_sync(self):
        ...
        try:
            ...
            try:
                ...
                new_questions = (
                    db.query(Question).filter(Question.created_at >= yesterday).all()
                )
                ...
                for question in new_questions:
                    ...
                    if not success:
                        logger.warning(
                            "failed_to_store_embedding", question_id=question.id
                        )
                ...
                for question in updated_questions:
                    ...
                    if not success:
                        logger.warning(
                            "failed_to_update_embedding", question_id=question.id
                        )
                ...
                logger.info(
                    "daily_embedding_sync_completed", task_id=task_id, **task.result
                )
            ...
            logger.error("daily_embedding_sync_failed", task_id=task_id, error=str(e))
            raise
    ...
    async def weekly_full_reindex(self):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "weekly_full_reindex_completed", task_id=task_id, **task.result
                )
            ...
            logger.error("weekly_full_reindex_failed", task_id=task_id, error=str(e))
            raise
    ...
    async def cleanup_expired_cache(self):
        ...
        try:
            ...
            logger.info("cache_cleanup_completed", task_id=task_id, **result)

        except Exception as e:
            await self._update_stats("cleanup_expired_cache", "error")
            logger.error("cache_cleanup_failed", task_id=task_id, error=str(e))
    ...
    async def trigger_model_migration(self, new_model_key: str, force: bool = False):
        ...
        try:
            ...
            logger.error(
                "model_migration_schedule_failed", task_id=task_id, error=str(e)
            )
            raise

    async def _execute_model_migration(
        self, task_id: str, new_model_key: str, force: bool
    ):
