from river import linear_model, preprocessing, compose, ensemble, tree, naive_bayes
import pickle
import json
import structlog
...
class EnhancedOnlineLearner:
    ...
    def predict_success_probability(self, user_features: dict, question_features: dict) -> float:
        ...
        try:
            ...
            logger.debug("success_probability_predicted", 
    ...
    def learn_from_answer(self, user_features: dict, question_features: dict, 
                         is_correct: bool, response_time: int, 
                         confidence_score: Optional[float] = None):
        """Öğrenci cevabından öğren"""
        try:
            combined_features = self._combine_features(user_features, question_features, response_time)
    ...
    def _combine_features(self, user_features: dict, question_features: dict, 
        ...
        features.update({
            ...
            'user_avg_time': min(user_features.get('avg_response_time', 5000) / 10000, 1.0),
            'user_topic_mastery': user_features.get('topic_mastery_math', 0.5),
            'user_consecutive': min(user_features.get('consecutive_correct', 0) / 10, 1.0),
            'user_session_count': min(user_features.get('session_question_count', 0) / 20, 1.0)
        })
        ...
        features.update({
            ...
            'question_avg_time': min(question_features.get('avg_response_time', 5000) / 10000, 1.0),
            'question_attempts': min(question_features.get('attempt_count', 0) / 100, 1.0)
        })
    ...
    def _update_performance_metrics(self, is_correct: bool, response_time: int):
        ...
        try:
            ...
            if metrics['total_predictions'] > 0:
                ...
                metrics['avg_response_time'] = metrics['total_response_time'] / metrics['total_predictions']
