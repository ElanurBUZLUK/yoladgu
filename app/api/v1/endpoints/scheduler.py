...
async def trigger_model_migration(request: ModelMigrationRequest):
    ...
    try:
        ...
        task_id = await offline_scheduler.trigger_model_migration(
            new_model_key=request.new_model_key,
            force=request.force
        )
...
async def remove_job(job_id: str):
    ...
    try:
        ...
        protected_jobs = ['daily_embedding_sync', 'weekly_full_reindex', 'cleanup_expired_cache', 'update_vector_stats']
        if job_id in protected_jobs:
            raise HTTPException(status_code=403, detail="Cannot remove protected system job")
...
async def scheduler_health_check():
    ...
    try:
        ...
        is_running = offline_scheduler.scheduler.running if offline_scheduler.scheduler else False
        ...
        return {
            ...
            "jobs_count": len(offline_scheduler.scheduler.get_jobs()) if offline_scheduler.scheduler else 0,
            "timestamp": datetime.utcnow().isoformat()
        }
