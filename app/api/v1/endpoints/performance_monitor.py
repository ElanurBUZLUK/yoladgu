...
async def performance_dashboard():
    ...
    try:
        ...
        (vector_metrics, cache_metrics, rate_limit_metrics, 
         embedding_metrics, scheduler_metrics, health_metrics) = await asyncio.gather(*tasks)
...
async def get_cache_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        hit_ratio = stats.get('cache_hits', 0) / total_requests if total_requests > 0 else 0
...
async def get_embedding_service_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "stats": stats,
            "available_models": list(enhanced_embedding_service.available_models.keys()),
...
async def run_performance_benchmark():
    ...
    try:
        ...
        start = time.time()
        embeddings = await enhanced_embedding_service.compute_embeddings_batch_cached_async(test_texts)
        first_run_time = (time.time() - start) * 1000
        
        start = time.time()
        embeddings_cached = await enhanced_embedding_service.compute_embeddings_batch_cached_async(test_texts)
        ...
        benchmark_results["cache_performance"] = {
            ...
            "speedup_factor": round(first_run_time / second_run_time, 2) if second_run_time > 0 else "infinite",
        ...
        benchmark_results["embedding_computation"] = {
            ...
            "throughput_per_second": round(1000 / (sum(embedding_times) / len(embedding_times)), 2)
        }
        ...
        return {
            ...
            "timestamp": datetime.utcnow().isoformat(),
            "performance_summary": {
                "search_performance": "Excellent" if benchmark_results["vector_search"]["avg_latency_ms"] < 50 else "Good",
                "cache_effectiveness": "Excellent" if benchmark_results["cache_performance"]["speedup_factor"] > 5 else "Good",
