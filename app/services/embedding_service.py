...
class EmbeddingService:
    ...
    def save_embedding_to_db(self, question_id: int, embedding: List[float]) -> bool:
        ...
        try:
            ...
            logger.info(
                "embedding_saved", question_id=question_id, dimensions=len(embedding)
            )
            return True
    ...
    def save_embeddings_batch(
        self, question_embeddings: List[Tuple[int, List[float]]]
    ) -> int:
        ...
        try:
            ...
            # Batch update
            cursor.executemany(
                "UPDATE questions SET embedding = %s WHERE id = %s", json_embeddings
            )
    ...
    async def find_similar_questions(
        ...
        try:
            ...
            if not enhanced_embedding_service.vector_store_initialized:
                ...
                return await self._legacy_find_similar_questions(
    ...
    async def _legacy_find_similar_questions(
        self, query_embedding: List[float], threshold: float = 0.8, limit: int = 10
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            for row in results:
                try:
                    ...
                    logger.debug(
                        "embedding_parse_error", question_id=row["id"], error=str(e)
                    )
                    continue
    ...
    def get_embedding_stats(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "embedding_coverage": (questions_with_embedding / total_questions * 100)
    ...
    async def update_question_embedding(self, question_id: int, content: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.error(
                    "question_embedding_update_failed", question_id=question_id
                )
            ...
            logger.error(
                "update_question_embedding_error", question_id=question_id, error=str(e)
            )
            return False
    ...
    async def batch_update_embeddings(self, batch_size: int = 50) -> Dict:
        ...
        try:
            ...
            logger.info(
                "batch_embedding_update_completed",
...
async def find_similar_questions(
    query_embedding: List[float], threshold: float = 0.8, limit: int = 10
) -> List[Dict]:
    """OPTIMIZED: Vector store ile benzer soruları bul"""
    return await embedding_service.find_similar_questions(
        query_embedding, threshold, limit
    )
