...
class QuestionIngestionService:
    ...
    async def ingest_from_khan_academy(
        self, subject: str, topic: str, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def ingest_from_quizlet(
        ...
        try:
            ...
            url = "https://api.quizlet.com/2.0/sets/search"
            params = {"q": f"{subject} {topic}", "limit": limit}
            ...
            for set_data in data.get("sets", []):
                for card in set_data.get("terms", [])[:limit]:
                    question_data = {
                        ...
                        "options": [card.get("definition", "")]
                        + self._generate_fake_options(),
    ...
    async def scrape_from_website(
        self, url: str, subject: str, topic: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            question_elements = soup.find_all(
                ["div", "p"], class_=re.compile(r"question|soru|problem")
            )
            ...
            for element in question_elements:
                ...
                if len(question_text) > 20:  # Minimum uzunluk kontrolü
                    # Seçenekleri bul
                    options = []
                    option_elements = element.find_next_siblings(
                        ["div", "li"], class_=re.compile(r"option|choice|secenek")
                    )
    ...
    async def save_questions_to_database(
        self, db: Session, questions: List[Dict[str, Any]]
    ) -> int:
        """
    ...
    def _generate_fake_options(self) -> List[str]:
        ...
        return ["Seçenek A", "Seçenek B", "Seçenek C"]
