...
class SystemInitializationService:
    ...
    async def validate_system_health(self, db: AsyncSession) -> Dict[str, bool]:
        health_status = {
            "database": False,
            "redis": False,
            "pgvector_extension": False,
            "pgvector_column": False,
            "pgvector_index": False,
            "overall": False
        }
        ...
        # Pgvector checks (reusing existing methods)
        health_status["pgvector_extension"] = await self.check_pgvector(db)
        health_status["pgvector_column"] = await self.column_exists(db, "embedding")
        health_status["pgvector_index"] = await self.index_exists(db, self.index_name)
