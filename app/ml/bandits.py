...
class EnhancedLinUCBBandit:
        ...
        try:
            ...
            if is_exploration:
                self.stats["exploration_count"] = (
                    self.stats.get("exploration_count", 0) + 1
                )
        ...
        try:
            ...
            if self.stats["total_selections"] > 0:
                self.stats["avg_reward"] = (
                    self.stats["total_rewards"] / self.stats["total_selections"]
                )
    ...
    def _calculate_ucb(
        self, question_id: int, context_vector: np.ndarray
        ...
        try:
            ...
            confidence = self.alpha * np.sqrt(
                context_vector.T.dot(A_inv).dot(context_vector)
            )
            ...
            A_reg = self.A[question_id] + self.regularization * np.identity(
                self.feature_dim
            )
            theta = solve(A_reg, self.b[question_id])
            expected_reward = theta.dot(context_vector)
            confidence = self.alpha * np.sqrt(
                context_vector.T.dot(np.linalg.inv(A_reg)).dot(context_vector)
            )
            return (
                float(expected_reward + confidence),
                float(confidence),
